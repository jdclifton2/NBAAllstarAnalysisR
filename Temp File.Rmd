...
treeTest$was_mvp <- as.character(treeTest$was_mvp)
treeTest$was_mvp <- as.factor(treeTest$was_mvp)
...
importance(mvpForest)

a=c()
i=5
for (i in 3:8) {
  mvpForest2 <- randomForest(was_mvp ~ ., data = treeTrain, ntree = 500, mtry = i, importance = TRUE)
  predValid <- predict(mvpForest2, cleanDF, type = "class")
  a[i-2] = mean(predValid == cleanDF$was_mvp)
}
a
```
