...
Logistical
```{r logistic}
logFit <- glm(was_mvp ~ ., data = nonCorDF[trainNonCor, ], family = binomial(link = "logit"))
...
For this model, the overall accuracy is very high at $99.28699\%$. The model correctly predicted the MVP $81.81818\%$ of the time. At the same time, the model predicted a non-MVP as the MVP $0.6601568\%$ of the time.
...
We will now fit a random forest. 
```{r}
...
mvpForest$confusion
acc = table(predTest, treeTest$was_mvp)
acc
...
```{r logistic2}
logFit2 <- glm(was_mvp ~ VORP + WS + BPM + PER + PTS + AST + TRB + BLK + STL + TOV + GS , data = cleanDF[train, ], family = binomial(link = "logit"))
...
acuracy2 = mean(predLog2 == test$was_mvp)

logTable2 = table(predLog2, test$was_mvp)
logTable2

truePositive(logTable2)
falseNegative(logTable2)
f1Score(logTable2)
mcc(logTable2)
...
```{r logistic3}
logFit2 <- glm(was_mvp ~ VORP + WS + BPM + PER + PTS + AST + TRB + BLK + STL + TOV + GS , data = dfLT2000, family = binomial(link = "logit"))
...
predLog2 = ifelse(probLog2 > 0.5, TRUE, FALSE)
acuracy2 = mean(predLog2 == dfGT2000$was_mvp)

logTable2 = table(predLog2, dfGT2000$was_mvp)
logTable2
...
```{r logistic4}
logFit2 <- glm(was_mvp ~ VORP + WS * OWS * WS.48 + BPM + PER + PTS * FG. + AST + TRB + BLK + STL + TOV + GS , data = dfLT2000, family = binomial(link = "logit"))
...
mvpForest <- randomForest(was_mvp ~ VORP + WS * OWS * WS.48 + BPM + PER + PTS * FG. + AST + TRB + BLK + STL + TOV + GS, data = treeTrain, importance = TRUE)
mvpForest
...
dfLT2000$was_mvp <- as.character(dfLT2000$was_mvp)
dfLT2000$was_mvp <- as.factor(dfLT2000$was_mvp)

dfGT2000$was_mvp <- as.character(dfGT2000$was_mvp)
dfGT2000$was_mvp <- as.factor(dfGT2000$was_mvp)

mvpForest <- randomForest(was_mvp ~ ., data = dfLT2000, importance = TRUE)
mvpForest

varImpPlot(mvpForest, sort=TRUE)

predTest <- predict(mvpForest, dfGT2000, type = "class")

mvpForest$importance
mvpForest$confusion
acc = table(predTest, dfGT2000$was_mvp)
acc
